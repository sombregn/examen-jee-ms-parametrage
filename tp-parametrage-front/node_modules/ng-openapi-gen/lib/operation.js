...
class Operation {
    ...
    collectResponses() {
        ...
        successResponse = (_a = responseByType.get('successResponse')) !== null && _a !== void 0 ? _a : responseByType.get('defaultResponse');
            ...
            if (content && content.length > 0) {
                for (const type of content) {
                    if (type && type.mediaType) {
                        ...
                        if (map.has(part)) {
                            this.logger.warn(`Overwriting variant method part '${part}' for media type '${(_a = map.get(part)) === null || _a === void 0 ? void 0 : _a.mediaType}' by media type '${type.mediaType}'.`);
                        }
                        map.set(part, type);
                    }
                }
        ...
        if (Array.isArray(this.options.keepFullResponseMediaType)) {
            for (const check of this.options.keepFullResponseMediaType) {
                if (check.mediaType === undefined || new RegExp(check.mediaType).test(mediaType)) {
                    return (_a = check.use) !== null && _a !== void 0 ? _a : 'short';
                }
            }
        }
        return 'short';
